...
class Movement(object):
    ...
    def get_available_moves(self):
        ...
        if self.piece.is_king():
            king_moves = self._add_moves_for_col(self.row - self.origin.value, -1, -self.origin.value)\
                       + self._add_moves_for_col(self.row - self.origin.value, +1, -self.origin.value)
        
        return king_moves\
             + self._add_moves_for_col(self.row + self.origin.value, -1, self.origin.value)\
             + self._add_moves_for_col(self.row + self.origin.value, +1, self.origin.value)
        
    def _add_moves_for_col(self, row, col_dir, row_dir):
        new_col = self.col + col_dir
        if self.board.valid_position(row, new_col) and self.board.get_piece(row, new_col) is None:
            return [[(row, new_col)]]
        elif self.board.get_piece(row, new_col) is not None and self.board.get_piece(row, new_col).get_origin() != self.origin:
            return self._start_recursion(row, new_col, col_dir, row_dir)
            
        return []
        
    def _start_recursion(self, row, col, col_dir, row_dir):
        '''
        ...
        self._recurse(new_new_row, new_col, -1, row_dir, move, moves)
        self._recurse(new_new_row, new_col, +1, row_dir, move, moves)
        ...
        if self.piece.is_king():
            new_new_row = new_row - row_dir
            self._recurse(new_new_row, new_col, col_dir, -row_dir, move, moves)
        
        return moves
    
    def _recurse(self, row, col, col_dir, row_dir, move, moves):
        if self.board.get_piece(row, col + col_dir) is not None and self.board.get_piece(row, col + col_dir).get_origin() != self.origin:
            new_moves = self._start_recursion(row, col + col_dir, col_dir, row_dir)
